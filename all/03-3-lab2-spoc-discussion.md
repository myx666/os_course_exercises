...
不同特权级能访问的内容不同。ring0最高，ring3最低。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

CPL是代码特权级；DPL是被访问段的特权级；RPL是访问者的特权级。

高优先级的能访问低优先级的；访问门时需要CPL <= DPL[门] & CPL >= DPL[段]；访问段时需要max(CPL,RPL) <= DPL[段]。
...
访问数据时对访问数据段的特权级判断；产生中断时与门特权级的判断；RPL与CPL的判断。
...
ring3时多压了SS与ESP；不同特权级下栈的状态切换也不同。

4. 如何利用int和iret指令完成不同特权级的切换？

使用int访问，iret修改返回的cs特权级。
...
<font color="#dd0000">3次：指令地址，源操作数地址，目的操作数地址。</font>

2. 描述X86-32的MMU地址转换过程；

虚拟地址先通过段机制转换成线性地址，然后通过页机制转换为物理地址。
...
跳转进入GDT的第二项（也就是GDT中的第二个描述符描述的地址0处的代码），执行选择子CODE_SEL=0x8。此处对应存放了内核代码的头512字节。
