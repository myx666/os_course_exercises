...
速度依次减小，容量依次增加，成本依次降低，访问频率依次降低。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

指计算机各种存储介质都处于相当的使用状态且不造成阻塞。
...
- 把程序分为若干功能相对独立的模块；不会同时执行的模块共享一块内存区域。
每组按照最大的来分配；
- 确定模块分组。
...
- 将暂时不允许的程序放到外存（整个进程为单位）；
- 交换是以进程为单位；覆盖是程序内部模块为单位。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

有。例如PASCAL的overlay就是覆盖。
...
- 时间局部性：相同的数据或者代码可能在一段时间内重复使用；
- 空间局部性：内存很可能使用一段附近空间的代码和数据；
- 分支局部性：分支跳转的地址很可能还是下一次分支跳转的地址。
...
只取当前部分进程内容放入物理内存。交换是整体，虚存是部分；覆盖是程序内，虚存没有指定模块与共享内存关系。
